...
public class JwtService { //creates/verifies and extracts info(claims) from jwt
    ...
    public JwtService(AuthProperties authProperties) {
        ...
        this.key = Keys.hmacShaKeyFor(jwtConfig.getSecret().getBytes(StandardCharsets.UTF_8)); //config string secret is converted into a byte arr using UTF-8 char encoding
        this.expirationMs = jwtConfig.getExpirationMinutes() * 60_000;
        this.tempExpirationMs = jwtConfig.getTempExpirationMinutes() * 60_000;
        this.refreshTokenExpirationMs = jwtConfig.getRefreshTokenDays() * 24 * 60 * 60 * 1_000;
    ...
    public String generateRefreshToken(String subject) {
        // Refresh token as simple JWT with longer expiration
        Map<String, Object> claims = Map.of("type", "refresh");
        return buildToken(subject, claims, refreshTokenExpirationMs);
    }
